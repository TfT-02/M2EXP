...
public class Buyfishing implements CommandExecutor
{
    ...
    {
        this.plugin = plugin;
        unitprice = this.plugin.getModuleConfig("fishing").getInt(
                "unitprice");
        expunits = this.plugin.getModuleConfig("fishing").getInt(
                "expunits");
                             ...
                             String commandLabel, String[] args)
    {
        try
        {
            if (cmd.getName().equalsIgnoreCase("bfishing")&&M2EXP.perms.has(sender, "m2exp.buy.fishing"))
            {

                if (sender instanceof Player)
                {
                    Player player = (Player) sender;
                  //  if (M2EXP.perms.has(player, "m2exp.buy.fishing"))
                    //{
