...
public class Buyswords implements CommandExecutor {
	...
	public Buyswords(M2EXP plugin) {
		this.plugin = plugin;
		 unitprice = this.plugin.getModuleConfig("swords").getInt(
					"unitprice");
		 expunits = this.plugin.getModuleConfig("swords").getInt(
					"expunits");
				...
				if (sender instanceof Player) {
					...
					if (args[0].equals("info")) {
						...
						if (r.transactionSuccess()) {
							...
							ExperienceAPI.addRawXP(player, SkillType.SWORDS.name(),
									u5);
