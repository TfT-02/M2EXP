...
public class Buyherbalism implements CommandExecutor {
	...
	public Buyherbalism(M2EXP plugin) {
		this.plugin = plugin;
		 unitprice = this.plugin.getModuleConfig("herbalism").getInt(
					"unitprice");
		 expunits = this.plugin.getModuleConfig("herbalism").getInt(
					"expunits");
				...
				if (sender instanceof Player) {
					...
					if (args[0].equals("info")) {
						...
						if (r.transactionSuccess()) {
							...
							ExperienceAPI.addRawXP(player, SkillType.HERBALISM.name(),
									u5);
